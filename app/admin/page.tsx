...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-6"><div className="mx-auto max-w-7xl space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-bold text-emerald-700">WorkLife WH Admin</p><h1 className="mt-1 text-2xl font-bold md:text-4xl">管理者ダッシュボード</h1><p className="mt-2 text-sm font-medium text-gray-700">利用状況、掲載申請、比較サービスの状況を確認できます。</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link href="/admin/launch-checklist" className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-center font-bold text-emerald-800">公開前チェック</Link><Link href="/admin/locations" className="rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-center font-bold text-blue-800">地域管理</Link><Link href="/admin/listings" className="rounded-md border border-gray-300 bg-white px-4 py-3 text-center font-bold text-gray-900">公開掲載を管理</Link><Link href="/admin/submissions" className="rounded-md bg-slate-900 px-4 py-3 text-center font-bold text-white">掲載申請 {(m?.jobs.pending ?? 0) + (m?.properties.pending ?? 0)}件</Link></div></header>
