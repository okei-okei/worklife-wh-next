...
export default function ApplyEmailPage() {
  ...
  const handleGenerate = () => {
    ...
    if (!selectedTarget) {
      setErrorMessage(
        targetType === "job"
          ? "応募メールを作成する求人を選択してください。"
          : "問い合わせメールを作成する物件を選択してください。",
      );
      return;
    }
  ...
  const handleCopy = async () => {
    ...
    if (!emailDraft.trim()) {
      setErrorMessage("コピーするメールがありません。");
      return;
    }

    try {
      await navigator.clipboard.writeText(emailDraft);
      setSuccessMessage("メール本文をコピーしました。");
    } catch {
  ...
  const handleSave = async () => {
    ...
    if (!emailDraft.trim()) {
      setErrorMessage("保存するメール本文がありません。");
      return;
    }
    ...
    const { error } = await supabase.from("application_documents").insert({
      ...
      document_type: "email",
      title: `${selectedTarget.title} - Application Email`,
      content: emailDraft,
    });
    ...
    if (error) {
      setErrorMessage(`メール本文の保存に失敗しました。${error.message}`);
      return;
    }

    setSuccessMessage("メール本文を保存しました。");
  };
          ...
          <section className="space-y-5 rounded-2xl bg-white p-4 shadow md:p-6">
            ...
            <label className="block">
              ...
              <textarea
                ...
                placeholder={
                  targetType === "job"
                    ? "求人を選択して応募メールを作成してください。"
                    : "物件を選択して問い合わせメールを作成してください。"
                }
