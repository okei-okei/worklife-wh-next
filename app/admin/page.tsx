...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-6"><div className="mx-auto max-w-7xl space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-bold text-emerald-700">WorkLife WH Admin</p><h1 className="mt-1 text-2xl font-bold md:text-4xl">管理者ダッシュボード</h1><p className="mt-2 text-sm font-medium text-gray-700">利用状況、掲載申請、比較サービスの状況を確認できます。</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link href="/admin/submissions" className="rounded-md bg-slate-900 px-4 py-3 text-center font-bold text-white">掲載申請 {(m?.jobs.pending ?? 0) + (m?.properties.pending ?? 0)}件</Link></div></header>
    ...
    <section><h2 className="mb-3 text-lg font-bold">人気コンテンツ</h2><div className="grid grid-cols-1 gap-4 lg:grid-cols-2"><Ranking title="人気比較サービス" items={m?.popularPartners || []} /><Ranking title="広告リンククリック上位" items={m?.popularAffiliateLinks || []} /></div></section>
