...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-6"><div className="mx-auto max-w-7xl space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-bold text-emerald-700">WorkLife WH Admin</p><h1 className="mt-1 text-2xl font-bold md:text-4xl">管理者ダッシュボード</h1><p className="mt-2 text-sm font-medium text-gray-700">利用状況、掲載、コンテンツ、リスクを横断して確認できます。</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link href="/admin/articles" className="rounded-md border border-gray-300 bg-white px-4 py-3 text-center font-bold">コラム管理 {m?.articles.pending ?? 0}件</Link><Link href="/admin/submissions" className="rounded-md bg-slate-900 px-4 py-3 text-center font-bold text-white">掲載申請 {(m?.jobs.pending ?? 0) + (m?.properties.pending ?? 0)}件</Link></div></header>
    ...
    <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      <Group title="記事" values={[["公開", m?.articles.published ?? 0], ["下書き", m?.articles.drafts ?? 0], ["承認待ち", m?.articles.pending ?? 0], ["却下", m?.articles.rejected ?? 0], ["総閲覧", m?.articles.totalViews ?? 0], ["比較ページ遷移", m?.articles.partnerTransitions ?? 0]]} />
      <Group title="リスク管理" values={[["通報", m?.risk.reports ?? 0], ["未対応通報", m?.risk.unresolvedReports ?? 0], ["削除済み投稿", m?.risk.deletedPosts ?? 0], ["承認待ち投稿", m?.risk.pendingPosts ?? 0], ["問い合わせ", m?.risk.contacts ?? 0], ["プライバシー申請", m?.risk.privacyRequests ?? 0]]} />
